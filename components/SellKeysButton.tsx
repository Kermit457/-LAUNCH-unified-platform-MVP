...
interface SellKeysButtonProps {
  curveId: string;
  twitterHandle: string;
  keys: number;
  solReceived: number; // SOL amount user will receive for selling
  userId: string;
  referrerId?: string;
...
export function SellKeysButton({
  curveId,
  twitterHandle,
  keys,
  solReceived,
  ...
  const { sellKeys, loading, error, txSignature, explorerUrl } = useSolanaSellKeys();
  ...
  const handleSell = async () => {
    try {
      ...
      const signature = await sellKeys(twitterHandle, solReceived, referrerId);
      console.log('✅ Solana sell tx confirmed:', signature);
      ...
      const data = await response.json();
      console.log('✅ Database updated after sell:', data);
    ...
    return (
      ...
      >
        {loading ? (
          ...
          children || `Sell ${keys} Key${keys !== 1 ? 's' : ''}`
        )}
      </button>
    );
  }
